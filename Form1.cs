...
{
    public partial class FTS00OII : Form
    {
        public FTS00OII()
        {
            InitializeComponent();
        ...
        private void FTS00OII_Load(object sender, EventArgs e)
        {
                    ...
                    using (SqlDataAdapter da = new SqlDataAdapter("", conn))
                    {
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            foreach (DataRow row in dt.Rows)
                            {
                                ...
                                string InputFilePath = row["InputFilePath"].ToString();
                                string InputProcessedFilePath = row["InputProcessedFilePath"].ToString();
                                string OutputFilePath = row["OutputFilePath"].ToString();

                                InvoiceIntegration.Execute(this, SvrType, Server, LicSvr, SQLLogin, SQLPass, SAPDBName, SAPLogin, SAPPass, 
        ...
        private void FTS00OII_FormClosing(object sender, FormClosingEventArgs e)
        {
